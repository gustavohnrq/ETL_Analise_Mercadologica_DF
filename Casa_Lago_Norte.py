...
def analisar_imovel_detalhado(df, oferta, tipo_imovel=None, bairro=None, cidade=None, cep=None, quadra=None, valor_limite=None):
    resultados_finais = []
    ...
    for vaga_status, df_vaga in df.groupby(df["vagas"].notnull()):
        ...
        if valor_limite is not None:
            df_vaga["Padrão"] = np.where(df_vaga["preco"] <= valor_limite, "Luxo", "SuperLuxo")
        else:
            df_vaga["Padrão"] = "N/A"
        
        for padrao, df_padrao in df_vaga.groupby("Padrão"):
            filtro = (df_padrao["oferta"] == oferta)
            if tipo_imovel:
                filtro &= (df_padrao["tipo"] == tipo_imovel)
            if bairro:
                filtro &= (df_padrao["bairro"] == bairro)
            if cidade:
                filtro &= (df_padrao["cidade"] == cidade)
            if cep:
                filtro &= (df_padrao["cep"] == cep)
            if quadra:
                filtro &= (df_padrao["quadra"] == quadra)

            df_filtrado = df_padrao[filtro]
...
for data in df['Data'].unique():
    ...
    resultados_detalhados = analisar_imovel_detalhado(
        ...
        cep=None,
        quadra=None,
        valor_limite= 3500000)  # Insira o valor limite desejado ou None para ignorar
